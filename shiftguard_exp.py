        ...
        for xw, xs, y in train_loader:
            ...
            with torch.autocast(device_type=device.type, dtype=torch.float16, enabled=device.type == "cuda"):
                ...
                if args.method == "erm":
                    ...
                    _, pm = forward_features(model, lam * xw + (1.0 - lam) * xw[perm])
                    loss = lam * ce(pm, y) + (1.0 - lam) * ce(pm, y[perm])
                else:
...
def parse_args():
    ...
    p.add_argument("--method", choices=["erm", "aug", "mixup", "feat", "feat_kl", "shiftguard"], default="shiftguard")
