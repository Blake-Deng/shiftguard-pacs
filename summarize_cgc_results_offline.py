...
def efficiency(rows: list[dict]) -> list[dict]:
    snapshot_path = SUMMARY_DIR / "table_efficiency.csv"
    if not snapshot_path.is_file():
        raise FileNotFoundError(
            "Efficiency checkpoints are omitted; committed table_efficiency.csv is required"
        )
    with snapshot_path.open(newline="", encoding="utf-8") as handle:
        snapshots = {row["method"]: row for row in csv.DictReader(handle)}
    output = []
    for method in ("strong_aug", "feature_plus_kl", "cgc"):
        ...
        if checkpoint.is_file() and torch.cuda.is_available():
            ...
            parameters = int(snapshot["parameters"])
            latency_ms = float(snapshot["batch1_inference_latency_ms"])
            inference_protocol = snapshot["inference_protocol"]
        output.append({
            "method": method,
