#!/usr/bin/env python3
"""Target-blind MixStyle formal matrix for the revision.

This runner uses the official MixStyle operation through the local adapter,
the shared source-validation checkpoint rule, and exactly one target evaluation.
...
def create_manifest(path: Path) -> list[dict]:
    ...
    for dataset, (data_root, targets) in DATASETS.items():
        for target in targets:
            for seed in SEEDS:
                exp = f"mixstyle-{dataset}-{target}-seed{seed}"
                out = ROOT / "runs" / "revision" / "mixstyle" / dataset / target / str(seed)
                stem = f"{dataset}_{target}_resnet50_mixstyle_seed{seed}"
                cfg = {"dataset": dataset, "target": target, "method": "mixstyle", "seed": seed, "p": 0.5, "alpha": 0.1, "val_fraction": 0.15, "epochs": 30, "batch_size": 64, "optimizer": "AdamW", "lr": 3e-4, "weight_decay": 1e-4, "checkpoint": "source_validation"}
                rows.append({"experiment_id": exp, "protocol_version": "shiftguard-mixstyle-v1.0", "dataset": dataset, "data_root": data_root, "outer_target": target, "method": "mixstyle", "seed": str(seed), "lambda_f": "0.0", "lambda_k": "0.0", "temperature": "2.0", "augmentation_M": "9", "selection_protocol": "highest_source_validation_accuracy_earliest_exact_tie", "config_hash": sha(cfg), "status": "pending", "result_path": str((out / f"{stem}.json").relative_to(ROOT)), "checkpoint_path": str((out / f"{stem}.pt").relative_to(ROOT)), "gpu": "", "exit_code": ""})
    if path.exists():
        with path.open(newline="", encoding="utf-8") as h:
            old = list(csv.DictReader(h))
        if [r["experiment_id"] for r in old] != [r["experiment_id"] for r in rows]:
            raise RuntimeError("existing MixStyle manifest IDs differ; refusing overwrite")
        return old
...
def command(row: dict) -> list[str]:
    return [sys.executable, str(ROOT / "shiftguard_multidataset.py"), "--dataset", row["dataset"], "--data-root", str(ROOT / row["data_root"]), "--target", row["outer_target"], "--method", "mixstyle", "--run-name", "mixstyle", "--seed", row["seed"], "--epochs", "30", "--batch-size", "64", "--workers", "8", "--image-size", "224", "--val-fraction", "0.15", "--lr", "0.0003", "--weight-decay", "0.0001", "--lambda-feat", "0.0", "--lambda-kl", "0.0", "--temperature", "2.0", "--warmup-epochs", "5", "--mixstyle-p", "0.5", "--mixstyle-alpha", "0.1", "--device", "cuda:0", "--output", str((ROOT / row["result_path"]).parent), "--exclusions", str(ROOT / "revision" / "cross_dataset_exclusions.json"), "--save-checkpoint"]


def validate(row: dict, result: dict) -> None:
    for key, expected in {"dataset": row["dataset"], "target": row["outer_target"], "method": "mixstyle", "run_name": "mixstyle", "model": "resnet50", "seed": int(row["seed"]), "target_evaluations": 1}.items():
...
def enrich(row: dict, cmd: list[str], gpu: int, start: str) -> None:
    ...
    result["audit"] = {"experiment_id": row["experiment_id"], "protocol_version": row["protocol_version"], "config_hash": row["config_hash"], "target_evaluations": 1, "target_accuracy": result["target_accuracy"], "mixstyle_source": "KaiyangZhou/mixstyle-release/reid/models/mixstyle.py", "mixstyle_p": 0.5, "mixstyle_alpha": 0.1, "source_only_selection": True, "start_time": start, "end_time": datetime.now(timezone.utc).isoformat()}
    tmp = rp.with_suffix(".audit.tmp"); tmp.write_text(json.dumps(result, indent=2), encoding="utf-8"); tmp.replace(rp)


def main() -> None:
    ap = argparse.ArgumentParser(); ap.add_argument("--manifest", default="revision/mixstyle_manifest.csv"); ap.add_argument("--gpu", nargs=2, type=int, default=(1, 2)); ap.add_argument("--dry-run", action="store_true")
    ...
    logs = ROOT / "logs" / "revision" / "mixstyle"; logs.mkdir(parents=True, exist_ok=True); lock = threading.Lock(); manifest_lock = threading.Lock(); failures = []
    def safe_write():
        with manifest_lock:
            write_manifest(path, rows)
    def worker(gpu: int):
