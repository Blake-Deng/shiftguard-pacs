...
def collect_samples(root: Path, target: str, seed: int, val_fraction: float):
    ...
    for domain, dpath in domain_dirs.items():
        for cls in classes:
            ...
            if not files:
                continue
            items = [(str(p), class_to_idx[cls], domain) for p in sorted(files)]
            if domain == target:
...
def coral_domain_loss(z, domains):
    groups = {}
    for i, d in enumerate(domains):
        groups.setdefault(str(d), []).append(i)
    covs = []
    for ids in groups.values():
        if len(ids) < 2:
            continue
        x = z[ids] - z[ids].mean(0, keepdim=True)
        covs.append((x.T @ x) / (len(ids) - 1))
    if len(covs) < 2:
        return z.new_zeros(())
    pairs = [(covs[i], covs[j]) for i in range(len(covs)) for j in range(i + 1, len(covs))]
    return sum((a - b).pow(2).mean() for a, b in pairs) / len(pairs)
...
def run(args):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for xw, xs, y, domains in train_loader:
                ...
                if args.method == "erm":
                    ...
                    if args.method == "aug":
                        ...
                        if args.method == "coral":
                            loss = lcls + args.lambda_feat * coral_domain_loss(zw, domains)
...
def parse_args():
    ...
    p.add_argument("--method", choices=["erm", "aug", "coral", "feat", "feat_kl", "shiftguard"], default="shiftguard")
